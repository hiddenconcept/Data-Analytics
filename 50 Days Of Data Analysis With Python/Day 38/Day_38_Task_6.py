...
# Find which car type is the most expensive to rent
# Using the average final price for each car type
car_type_price = df.groupby('Car Type')['Final Price'].mean()

most_expensive_car = car_type_price.idxmax()
highest_price = car_type_price.max()

print(f"\nMost expensive car type to rent: {most_expensive_car}")
print(f"Average rental price: {highest_price}")